...
class ProjectManager {
    ...
    async loadProjects(){
        ...
        for( let projectData of projects ){
            this.addProject( new Project(projectData), projectData._id );
        }
        this.loaded = true;
        ...
        models.Project.watch().on( 'change', async data => {
            ...
            const switchProject = ( oldProject, newProject ) => {
                
                if( oldProject.id == newProject.id ){
                    this.projects[oldProject.id] = newProject;

                } else {
                    this.removeProject( docID );
                    this.addProject( newProject, docID );
                }
            }
            ...
            switch( data.operationType ){
                ...
                case 'update':
                    ...
                    const oldProject = this.projects[this.projectIDs[docID]];
    ...
    addProject( project, docID ){
        this.projects[project.id] = project;
        this.projectIDs[docID] = project.id;
    }

    removeProject( docID ){
        const projectID = this.projectIDs[docID];
